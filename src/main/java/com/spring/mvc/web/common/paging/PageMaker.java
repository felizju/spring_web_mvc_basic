...
// 서버가 클라이언트에게 각종 페이지 정보를 전달하기 위해 페이징 데이터를 만들어주는 클래스
@Getter @ToString
public class PageMaker {
    ...
    private Criteria criteria; // 현재 요청 페이지 정보 (현재 페이지가 어디에 있는지)
    ...
    public PageMaker(Criteria criteria, int totalCount){
        ...
        // 끝 페이지 부터 계산
        // 올림 값 (현재 위치한 페이지 넘버 / 한 화면에 보여줄 페이지 수)  * 한 화면에 보여줄 페이지 수
        this.endPage = (int) Math.ceil((double) criteria.getPage()/PAGE_COUNT) * PAGE_COUNT;

        // 시작 페이지 계산
        // 끝 페이지 - 한 화면에 보여줄 페이지 수 + 1
        this.beginPage = endPage - PAGE_COUNT + 1;

        /*
            # 페이지 마지막 구간 끝 페이지 보정 #
              총 게시물 수가 284개이고, 한 화면당 10페이지씩 보여주고 있다면, 마지막 구간은 21-30이 아닌, 21-29가 되어야함
              따라서 마지막 구간에서 endPage 보정이 필요함
            # 보정 공식 : 올림 값 (총 게시물 수 / 한 페이지당 보여줄 게시물 수)
         */
        int realEnd = (int) Math.ceil((double) totalCount / criteria.getAmount());
        
        // 보정은 마지막 페이지 구간에서만 일어나야 함
        if(realEnd <= endPage){ // 29페이지 보다 같거나 클 경우에만
            this.endPage = realEnd;
        }
        
        // 이전 버튼 활성화 여부
        this.prev = this.beginPage > 1; // 1페이지 이상이면 이전 버튼이 무조건 활성화
