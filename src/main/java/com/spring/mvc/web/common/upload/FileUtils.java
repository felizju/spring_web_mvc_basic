...
public class FileUtils {
    ...
    public static MediaType getMediaType(String ext){
        ext = ext.toUpperCase(); // 소문자로도 가능하게 처리
    ...
    // 1. 사용자가 파일을 업로드 했을 때 저장 처리 및 파일명을 리턴하는 메서드
    public static String uploadFile(MultipartFile file, String uploadPath) throws IOException {
        ...
        /*
            ...
            이미지가 아니라면 그냥 다운로드할 수 있게 일반 파일 경로를 리턴
         */

        // 클라이언트가 서버에 있는 파일을 로딩하려면 해당 파일이 어디에 저장되어 있는지 정보가 필요하다
    ...
    private static String getNewUploadPath(String uploadPath) {
        ...
        for (String date : dateInfo){
            // File.separator : 운영체제에 맞게 경로부분 문자열을 만들어 줌
            // 윈도우 : \\ , 리눅스 : /
