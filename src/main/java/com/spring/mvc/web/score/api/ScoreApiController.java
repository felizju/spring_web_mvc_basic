...
public class ScoreApiController {
    ...
    @GetMapping("/{sn}")
    // @PathVariable
    public ResponseEntity<Score> selectOne(@PathVariable("sn") int stuNum){ // stuNum이면 생략 가능
        log.info("/api/score/"+stuNum+"GET!");
        return new ResponseEntity<>(scoreService.getDetail(stuNum), HttpStatus.OK);
    }

    // 점수 정보 등록 - 비동기 요청 처리
    @PostMapping("/Score-api-post")
    // @RequestBody : 클라이언트가 보낸 JSON 데이터를 파싱하여 자바 객체로 변환
    public ResponseEntity<String> save2(@RequestBody Score score){
        log.info("/api/score POST - " + score);
    ...
    public ResponseEntity<String> delete(@PathVariable int stuNum){
        log.info("/api/score/"+stuNum+"POST!");
        try{
