...
public class ApiControllerV2 {
    ...
    /*
        # ResponseEntity
            - 스프링 REST API 가 응답할 때 응답 상태 코드, 응답 헤더 등을 조작해서 전송할 수 있게 도와주는 객체
     */

    @GetMapping("/hello")
    public ResponseEntity<String> hello(String p){
        if(p.equals("hello")){
            return new ResponseEntity<>("안녕안녕", HttpStatus.OK);
        }else{
    ...
    @GetMapping("/board")
    public Board board(){
        return new Board(105,"김레스트","레스트api","레스트api 학습 중",0);
    }
